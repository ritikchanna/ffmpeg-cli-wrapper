...
public class FFmpegLoader {
  ...
  public static String FFMPEG_PATH = basePath + "/ffmpeg";
  public static String FFPROBE_PATH = basePath + "/ffprobe";
  public static String FFPLAY_PATH = basePath + "/ffplay";

  static {
    try {
      basePath = extractFFMPEG();
    } catch (Exception e) {
      e.printStackTrace();
  ...
  private static String extractFFMPEG() throws IOException, InstantiationException {
    ...
    } else throw new InstantiationException(name + " : " + arch + " not supported.");
  }

  private static String extractResource(String os) throws IOException {
    final File jarFile =
        new File(FFmpegLoader.class.getProtectionDomain().getCodeSource().getLocation().getPath());
    String path;
    if (jarFile.isFile()) { // Run with JAR file
      ...
      final URL url = FFmpegLoader.class.getResource("/bin/" + os);
      path = url.getPath();
    }
    return path;
  }
  ...
  private static boolean copyFile(String srcFile, String destFolder) throws IOException {
    ...
    try (InputStream inputStream = FFmpegLoader.class.getResourceAsStream(srcFile);
        OutputStream outputStream =
            new FileOutputStream(
                destFolder + "/" + srcFile.substring(srcFile.lastIndexOf('/') + 1)); ) {
      int readBytes;
